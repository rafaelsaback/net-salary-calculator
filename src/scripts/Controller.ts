...
import { B2BCalculator, B2BOptions, TAXTYPE, ZUS } from './B2BCalculator';
import {
  switchPanel,
  updateHeaderNames,
...
export const btnB2BOptions: HTMLButtonElement = document.querySelector(
  '#btn-b2b-options',
);
export const btnCalculate: HTMLButtonElement = document.querySelector(
  '#btn-calculate',
);

// Input elements
const inputCosts: HTMLInputElement = document.querySelector('#costs');
const inputSalary: HTMLInputElement = document.querySelector(
  '#input-gross-salary',
);
...
const rdoTaxProgressive: HTMLInputElement = document.querySelector(
  '#tax-progressive',
);
...
const rdoDiscountedZUS: HTMLInputElement = document.querySelector(
  '#discounted-zus',
);
const rdoNormalZUS: HTMLInputElement = document.querySelector('#normal-zus');
const rdoSicknessYes: HTMLInputElement = document.querySelector(
  '#sickness-yes',
);
...
var calculateThenScroll = function(selectedContract: symbol): void {
...
inputSalary.focus();
btnUOP.addEventListener('click', () => {
  selectContract(uopCalculator);
});
btnB2B.addEventListener('click', () => {
  selectContract(b2bCalculator);
});
btnUOP.click();
btnCalculate.addEventListener('click', () => {
  calculateThenScroll(selectedContract);
});
inputSalary.addEventListener('keydown', event => {
  pressedKey(event, selectedContract);
});
inputCosts.addEventListener('keydown', event => {
  pressedKey(event, selectedContract);
});
btnB2BOptions.addEventListener('click', () => {
  toggleB2BOptions();
});

// Automatically calculate when changing B2B options (only it has been already calculated once)
rdoVat0.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoVat5.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoVat8.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoVat23.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoTaxProgressive.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoTaxLinear.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoNoZUS.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoDiscountedZUS.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoNormalZUS.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoSicknessYes.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
rdoSicknessNo.addEventListener('click', () => {
  if (isCalculated) calculate(selectedContract);
});
