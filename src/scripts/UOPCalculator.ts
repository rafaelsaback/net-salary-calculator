...
export class UOPCalculator extends BaseCalculator {
  private options: AuxVariable = {};
  public constructor(options: AuxVariable) {
    super();
  ...
  public calcSalary(grossSalary: number): void {
    this.monthly.grossSalary.fill(grossSalary);

    // Accumulated gross salary
    this.monthly.accGrossSalary = this.calcAccGrossSalary(
      this.monthly.grossSalary,
    );

    // Pension
    this.monthly.pension = this.calcPension(
      this.monthly.grossSalary,
      this.monthly.accGrossSalary,
      this.options.annualLimit,
      this.options.pension,
    );
    ...
    this.monthly.disability = this.calcDisability(
      ...
      this.monthly.accGrossSalary,
      this.options.annualLimit,
      this.options.disability,
    );

    // Sickness insurance
    this.monthly.sickness = this.calcSickness(
      this.monthly.grossSalary,
      this.options.sickness,
    );
    ...
    // Net salary
    this.monthly.netSalary = super.calcFinalSalary(
      this.monthly.grossSalary,
      this.monthly.socialSecurity,
      this.monthly.healthContribution,
      this.monthly.tax,
  ...
  private calcAccGrossSalary(grossSalary: number[]): number[] {
    return this.accumulateValue(grossSalary);
  }

  private calcPension(
    grossSalary: number[],
    accGrossSalary: number[],
    annualLimit: number,
    pensionRate: number,
  ): number[] {
    return this.calcPensionDisability(
      grossSalary,
      accGrossSalary,
      annualLimit,
      pensionRate,
  ...
  private calcSickness(grossSalary: number[], sicknessRate: number): number[] {
  ...
  private calcHealthContribution(
    grossSalary: number[],
    socialSecurity: number[],
    rateHealthContribution: number,
  ): number[] {
    ...
    for (let i = 0; i < healthContribution.length; i++) {
      ...
      healthContribution[i] = this.calcContribution(
        healthBase,
        rateHealthContribution,
      );
    }
